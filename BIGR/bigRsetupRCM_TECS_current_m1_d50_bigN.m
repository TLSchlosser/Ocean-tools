...
filein(12) = -3.1e-7*.8;%dispcurve(1,1);%-2*pi/200e5;%
...
% nsqr=N2_days(~isnan(N2_days(:,3)),3)./min(N2_days(:,3)).*1.5.*min(N2_days(:,3));%
nsqr=N2_0_bigN(~isnan(N2_0_bigN(:,3)),3);
...
zmax = max(depr);
if vzero ~= 0
    xzero = Cmean.xzero;%input(' Input distance offshore to peak mean flow (km) ');
    zzero = 0;%input(' Input depth of peak mean flow (m) ');
    zscaled = 250;%Cmean.zscaled;%input(' Downward exponential scale of mean flow? (m) ');
    zscaleup = 200;%input(' Upward exponential scale of mean flow? (m) ');
    xscaleoff = Cmean.xscaleoff;%input(' Offshore exponential scale of mean flow? (km) ');
    xscaleon = Cmean.xscaleon;%input(' Onshore exponential scale of mean flow? (km) ');
...
pause(5)
movefile('uvwrp.mat',['BIGR_D1_Mode',num2str(mode),'X_r07_d50_bigN_check.mat'])
Cx=x;Cz=z;
save(['BIGR_D1_Mode',num2str(mode),'X_r07_d50_bigN_check.mat'],'Cx','Cz','vtemp','-append')
