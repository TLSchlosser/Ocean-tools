function modtowCO(command,parameter)
...
if command == 0,  % then initialize the menus/number
   ...
   h_menu_typeCO=uicontrol('Style','popupmenu',...
      'Callback','modtowCO(3)','FontSize',fs,...
...
elseif command == 2, % delete an element
   ...
   if deleleCO <= mb,  % then we'll remove element number delele
      ...
      Iojb=[];Jobj=[];Pobj=[];
   	...
   	for ico=1:mmCO,
   	   for i=1:mm-1,
   	      if ZCO(ico)>sum(Ht(1,1:i)) & ZCO(ico)<=sum(Ht(1,1:(i+1))),
   	         Jobj(ico)=i+1; % moorele object onto which we've clamped
   	         dz=ZCO(ico)-sum(Ht(1,1:i));
   	         Pobj(ico)=dz/Ht(1,i+1); % percentage of length along Jobj where we've clamped.
   	         i=mm-1;
   	      end
   	   end
   	end
